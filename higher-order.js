...
let orderTotals = orders.map(cost => {
  return cost.price * cost.tax + cost.price;
});
...
let bobs = purchases.filter(e => {
  return e.owner && e.owner !== `Barry`;
});

let bobsTotal = bobs.reduce((sum, e) => {
  return sum + e.price;
}, 0);
